...
class nac(MetricsTime):
    def __init__(self, test, input, layers, t=0):
        ...
        for index, l in layers:
            ...
            if index == 'conv':
                ...
                with tf.compat.v1.Session() as sess:
                    temp = sess.run(l, feed_dict={input: test})
                temp = temp.reshape(len(test), -1, l.shape[-1])
        ...
        e = time.time()
        self.t_collection += e - s
    def get_nue_cov_vector(self):
        neuron_metrix = np.concatenate(self.neuron_activate, axis=1)
        nue_bool_metrix = (neuron_metrix>self.t)
        nue_bool_metrix2 = (neuron_metrix<1-self.t)
        n=0
        nue_bool_metrix3=[]
        for i in neuron_metrix:
            for j in i:
                if j<0:
                    n+=1
                    nue_bool_metrix3.append(j)
        # print("nue_bool_metrix3,",n)
        # print("nue_bool_metrix3,",nue_bool_metrix3)

        nue_bool_metrix = np.concatenate([nue_bool_metrix,nue_bool_metrix2],axis=0)
        # print('nue_bool_metrix',nue_bool_metrix)
        result = np.bitwise_or.reduce(nue_bool_metrix, axis=0).astype(int)
        # print('result',result)
    ...
    def fit(self):
        ...
        # for neu in self.neuron_activate:
        #     vec = []
        #     activate_nums = np.sum(neu > self.t, axis=0)
        #     vec = np.where(activate_nums > 0, 1, 0)
        #     neuron_activate += np.sum(activate_nums > 0)
        #     activate_vec.append(vec.copy())
        #     cur_seed_nac_vec = [item for array in activate_vec for item in array]
        #     cur_seed_nac_vec = [int(x) for x in cur_seed_nac_vec]
        # return neuron_activate / float(self.neuron_num), cur_seed_nac_vec
        vector = self.get_nue_cov_vector()
        rate = np.sum(vector) / float(self.neuron_num)
        return rate, vector.tolist()
