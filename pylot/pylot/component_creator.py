...
def add_obstacle_detection(center_camera_stream,
    ...
    #将center_camera_stream换成自己的数据流

    if FLAGS.obstacle_detection:
   
        # 使用新的 ObstacleDetectionStream
        # custom_image_folder = '/home/lzq/traffic_light'  # 替换为您的数据集路径
        # (custom_obstacles_stream, _) = pylot.operator_creator.add_custom_obstacle_image_stream(custom_image_folder,
        #                                                                         center_camera_stream,tl_transform)
...
def add_traffic_light_detection(tl_transform,
    ...
    if FLAGS.traffic_light_detection:
        ...
        # custom_image_folder = '/home/lzq/experiment_datatset/inputs/traffic_light'  # 替换为您的数据集路径
        # (custom_image_stream, _) = pylot.operator_creator.add_custom_image_stream(custom_image_folder,
        #                                                                         tl_camera_stream,
        #                                                                         tl_camera_setup)
        # # print("Custom Image Stream:", custom_image_stream)

        # traffic_lights_stream = \
        #     pylot.operator_creator.add_traffic_light_detector(
        #         custom_image_stream, time_to_decision_stream)
...
def add_obstacle_tracking(center_camera_stream,
    ...
    if FLAGS.obstacle_tracking:
        if FLAGS.tracker_type == 'center_track':
            print('Using CenterTrack obstacle tracker...')
            ...
            print('Using QDTrack obstacle tracker...')
            obstacles_wo_history_tracking_stream = \
                pylot.operator_creator.add_qd_track_tracking(
                    center_camera_stream, center_camera_setup)
        else:
            print('Using obstacle tracker...')
            obstacles_wo_history_tracking_stream = \
    ...
    if FLAGS.perfect_obstacle_tracking:
        assert (pose_stream is not None
                and ground_obstacles_stream is not None)
        print('Using perfect obstacle tracker...')
        obstacles_tracking_stream = \
