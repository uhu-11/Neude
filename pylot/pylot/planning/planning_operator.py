...
class PlanningOperator(erdos.Operator):
    ...
    def on_watermark(self, timestamp: erdos.Timestamp,
        ...
        # else:
        # if self._world.waypoints is not None:
        (speed_factor, _, _, speed_factor_tl,
        speed_factor_stop) = self._world.stop_for_agents(timestamp)
        
        # else:
        #     speed_factor,speed_factor_tl, speed_factor_stop = 1,1,1
        if self._flags.planning_type == 'waypoint':
            ...
            output_wps = self._planner.run(timestamp, ttd)
            # print('在应用speed_factor之前的wps：', output_wps)
        ...
        # print('output_wps,',output_wps)
        output_list = [[output_wps.as_numpy_array_2D()[0][i], output_wps.as_numpy_array_2D()[1][i]] for i in range(len(output_wps.as_numpy_array_2D()[0]))]
        np.save(f'/media/lzq/D/lzq/pylot_test/pylot/predictions/planning/planning_{self._iter:08d}.npy', output_list)
        self._iter+=1
        waypoints = WaypointsMessage(timestamp, output_wps)
        # logging.warning('waypointshhhhhh:{},{}'.format(waypoints, output_list))
        waypoints_stream.send(waypoints)
    @coverage_decorator
    ...
    def update_world(self, timestamp: erdos.Timestamp):
        ...
        #     prediction_msg=PredictionMessage(timestamp, [])
        # print('传到plan里面的感知识别信息:', prediction_msg)
