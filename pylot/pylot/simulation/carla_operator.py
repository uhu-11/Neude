...
import erdos
from erdos import ReadStream, Timestamp, WriteStream, Message, WatermarkMessage
...
from pylot.recorder_and_replayer.recorder_replayer import SceneDataReplayer, SceneDataRecorder, HeroVehicleDataReplayer,  HeroVehicleDataRecorder, WorldDataRecorder, WorldDataReplayer
...
class CarlaOperator(erdos.Operator):
    ...
    def __init__(
        ...
        # random.seed(42)
        # self._client.get_trafficmanager().set_random_device_seed(42)
        ...
        # 添加回放模式相关的属性
        self.replay_mode = self._flags.replay_mode
        if self.replay_mode:
            self.scene_replayer = SceneDataReplayer(self._world,self._flags.record_file)
            self.hero_replayer = HeroVehicleDataReplayer(self._flags.hero_vehicle_data_record_file)
            self.world_replayer = WorldDataReplayer(self._flags.world_data_record_file)
        else:
            self.scene_recorder = SceneDataRecorder(self._world,self._flags.record_file)
            self.hero_recorder = HeroVehicleDataRecorder(self._flags.hero_vehicle_data_record_file)
            self.world_recorder = WorldDataRecorder(self._flags.world_data_record_file)
    @staticmethod
    @coverage_decorator
    ...
    def on_pipeline_finish(self, timestamp: Timestamp):
        ...
        if not self.replay_mode:
            self.scene_recorder.save_recorded_scenes()
        # 调用原有的清理逻辑
        if self._client:
    ...
    def __send_hero_vehicle_data(self, stream: WriteStream,
                                 timestamp: Timestamp):
        if self.replay_mode:
            pose = self.hero_replayer.get_scene_data(timestamp, self._flags.seed_ind)
        else:
            vec_transform = pylot.utils.Transform.from_simulator_transform(
                self._ego_vehicle.get_transform())
            velocity_vector = pylot.utils.Vector3D.from_simulator_vector(
            ...
            pose = pylot.utils.Pose(vec_transform, forward_speed, velocity_vector,
                                    timestamp.coordinates[0])
            self.hero_recorder.record_scene(timestamp, pose)
        # vec_transform = pylot.utils.Transform.from_simulator_transform(
        #         self._ego_vehicle.get_transform())
        # velocity_vector = pylot.utils.Vector3D.from_simulator_vector(
        #     self._ego_vehicle.get_velocity())
    ...
    def __send_ground_actors_data(self, timestamp: Timestamp):
        ...
        if self.replay_mode:
            # 回放模式：从记录中获取数据
            scene_data = self.scene_replayer.get_scene_data(timestamp, self._flags.seed_ind)
            if scene_data is None:
                return
                
            vehicles, people, traffic_lights, speed_limits, traffic_stops = scene_data
        else:
            # 记录模式：从模拟器获取数据并记录
            actor_list = self._world.get_actors()
            ...
            # 记录场景数据
            self.scene_recorder.record_scene(timestamp, vehicles, people, traffic_lights, speed_limits, traffic_stops)
    ...
    def __send_world_data(self):
        ...
        # attach sensors.
        if self.replay_mode:
            open_drive_data=self.world_replayer.get_scene_data()['open_drive_data']
            vehicle_id_data=erdos.Message(erdos.Timestamp(coordinates=[0]),
                            self._ego_vehicle.id)
            
        else:
            vehicle_id_data=erdos.Message(erdos.Timestamp(coordinates=[0]),
                            self._ego_vehicle.id)
            open_drive_data=erdos.Message(erdos.Timestamp(coordinates=[0]),
                            self._world.get_map().to_opendrive())

            datas = {'open_drive_data':open_drive_data,'vehicle_id_data':vehicle_id_data}
            self.world_recorder.record_scene(datas)
