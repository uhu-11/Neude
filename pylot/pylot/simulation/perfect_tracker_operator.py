...
class PerfectTrackerOperator(erdos.Operator):
    ...
    def on_watermark(self, timestamp: Timestamp,
        ...
        print('路径跟踪模块接收到的障碍物信息：', obstacles_msg)
        print('路径跟踪模块接收到的pose_msg：', pose_msg)
        ...
        # timestamp.
        # self._vehicle_id=3279
        for obstacle in obstacles_msg.obstacles:
            print(f'self._vehicle_id, obstacle.id: {self._vehicle_id}, {obstacle.id}')
            if obstacle.id == self._vehicle_id and not \
               self._flags.prediction_ego_agent:
                ...
                print(f'obstacle.id == self._vehicle_id and not self._flags.prediction_ego_agent')
                continue
            ...
            if (pose_transform.location.distance(obstacle.transform.location) >
                ...
                print('pose_transform.location.distance(obstacle.transform.location) >self._flags.dynamic_obstacle_distance_threshold')
                continue
        ...
        print(f'完美路径跟踪模块输出的长度：{len(obstacle_trajectories)}')
        print(f'{timestamp}完美路径跟踪模块输出的路径：',obstacle_trajectories)
