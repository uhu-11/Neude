...
class MPCOperator(erdos.Operator):
    ...
    def on_watermark(self, timestamp, control_stream):
        ...
        data_map = {'steer': control_msg.steer, 'throttle':control_msg.throttle,
                    'brake': control_msg.brake, 'hand_brake':control_msg.hand_brake,
                    'reverse': control_msg.reverse}
        logging.warning('control_msg:{}'.format(data_map))
        np.save(f'/media/lzq/D/lzq/pylot_test/pylot/predictions/control/control_{self._iter:08d}.npy', data_map)
        self._iter+=1
