...
from pylot.recorder_and_replayer.recorder_replayer import PointCloudRecorder, PointCloudReplayer
...
class CarlaLidarDriverOperator(erdos.Operator):
    ...
    def __init__(self, ground_vehicle_id_stream: erdos.ReadStream,
        ...
        if self.replay_mode:
            self.point_cloud_replayer = PointCloudReplayer(self._flags.point_cloud_record_file)
        else:
            self.point_cloud_recorder = PointCloudRecorder(self._flags.point_cloud_record_file)
            ...
            with self._lock:
                ...
                if self.replay_mode:
                    msg = PointCloudMessage(
                        timestamp,
                        self.point_cloud_replayer.get_scene_data(timestamp,self._flags.seed_ind))
                else:
                    pfc=PointCloud.from_simulator_point_cloud(
                            simulator_pc, self._lidar_setup)
                    msg = PointCloudMessage(
                        timestamp,
                        pfc)
                    self.point_cloud_recorder.record_scene(timestamp, pfc)
