...
def add_traffic_light_detector(traffic_light_camera_stream,
    ...
    [traffic_lights_stream
                       ...
                       FLAGS)
    logging.warning("have connect TrafficLightDetOperator")
...
def add_camera_driver(camera_setup, vehicle_id_stream, release_sensor_stream):
    ...
    return erdos.connect(CarlaCameraDriverOperator, op_config,
                         ...
                         camera_setup, FLAGS)
# pylot/operator_creator.py
def add_custom_image_stream(image_folder, tl_camera_stream, tl_camera_setup, fps=30):
    from pylot.streams.CustomImageStream import CustomImageStream
    op_config = erdos.OperatorConfig(name='custom_image_stream_operator',
                                     flow_watermarks=False,
                                     log_file_name=FLAGS.log_file_name,
                                     csv_log_file_name=FLAGS.csv_log_file_name,
                                     profile_file_name=FLAGS.profile_file_name)
    ...
    # 连接操作符并返回流
    camera_stream, notify_reading_stream = erdos.connect(CustomImageStream, op_config,
                                                         [tl_camera_stream],
                                                         image_folder, tl_camera_setup, fps)
    
    return camera_stream, notify_reading_stream  # 返回两个流

def add_custom_obstacle_image_stream(image_folder, tl_camera_stream, center_camera_setup, fps=30):
    from pylot.streams.CustomImageStream_obstacle import CustomImageStream_obstacle
    op_config = erdos.OperatorConfig(name='custom_obstacle_image_stream_operator',
                                     flow_watermarks=False,
                                     log_file_name=FLAGS.log_file_name,
                                     csv_log_file_name=FLAGS.csv_log_file_name,
                                     profile_file_name=FLAGS.profile_file_name)
    ...
    # 连接操作符并返回流
    camera_stream, notify_reading_stream = erdos.connect(CustomImageStream_obstacle, op_config,
                                                         [tl_camera_stream],
                                                         image_folder,center_camera_setup, fps)
