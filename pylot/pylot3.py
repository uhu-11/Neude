...
flags.DEFINE_boolean('replay_mode', False, 'Whether to run in replay mode')
flags.DEFINE_string('record_file', 'scene_data.pkl', 'Path to save/load scene data')
flags.DEFINE_integer('fixed_seed', None, 'Fixed random seed for reproducibility.')
flags.DEFINE_string('hero_vehicle_data_record_file', 'hero_vehicle_data_recorded_scenes.pkl', 'Path to save/load hero vehicle data')
flags.DEFINE_string('world_data_record_file', 'world_data_recorded_scenes.pkl', 'Path to save/load world data')
flags.DEFINE_string('point_cloud_record_file', 'point_cloud_record_file.pkl', 'Path to save/load point_cloud data')
flags.DEFINE_integer('seed_ind',0, 'seedindex')
...
def driver():
    ...
    custom_image_folder = '/home/lzq/experiment_datatset/inputs/obstacles'  # 替换为您的数据集路径
    (custom_obstacles_stream, _) = pylot.operator_creator.add_custom_obstacle_image_stream(custom_image_folder,
    ...
    obstacles_stream, perfect_obstacles_stream = \
        pylot.component_creator.add_obstacle_detection(
            custom_obstacles_stream, center_camera_setup, pose_stream,
            depth_stream, depth_camera_stream, ground_segmented_stream,
            ground_obstacles_stream, ground_speed_limit_signs_stream,
            ground_stop_signs_stream, time_to_decision_loop_stream,tl_transform)
    ...
    lane_detection_stream = pylot.component_creator.add_lane_detection(
        custom_obstacles_stream, pose_stream, open_drive_stream)
    ...
    obstacles_tracking_stream = pylot.component_creator.add_obstacle_tracking(
        custom_obstacles_stream, center_camera_setup, obstacles_stream,
    ...
    segmented_stream = pylot.component_creator.add_segmentation(
        custom_obstacles_stream, ground_segmented_stream)
    ...
    if pylot.flags.must_visualize():
        control_display_stream, ingest_streams = \
            pylot.operator_creator.add_visualizer(
                pose_stream, custom_obstacles_stream, tl_camera_stream,
...
def main(args):
    ...
    # 解析命令行参数
    # with open('/media/lzq/D/lzq/pylot_test/error_log2.txt', 'a') as f:
    #     sys.stderr = f
        flags.FLAGS(args)  # 解析参数
...
def run_pylot_with_flags(flagfile):
    # with open('/media/lzq/D/lzq/pylot_test/error_log_fuzz.txt', 'a') as f:
    #     sys.stderr = f
         # 通过指定的 flagfile 运行 Pylot
        flags.FLAGS(['pylot2.py', '--flagfile', flagfile])  # 加载参数文件
        main(['pylot2.py']) 
        # inputs.image = image
...
# if __name__ == '__main__':
#     config_path = '/media/lzq/D/lzq/pylot_test/pylot/configs/detection.conf'
