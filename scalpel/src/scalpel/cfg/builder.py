...
class CFGBuilder(ast.NodeVisitor):
    ...
    def build(self, name, tree, asynchr=False, entry_id=0, flattened=False):
        ...
        self.visit(tree)
        # print("builder_cfg", self.cfg.all_variable)
        visited = []
        self.clean_cfg(self.cfg.entryblock, visited)

        if flattened:
            self.cfg = self.cfg.flatten()

        # self.cfg.getFucFirstRow()
        # self.cfg.getClassFirstRow()
        self.cfg.getFuctionCallRow()
        self.cfg.getClassCallRow()
    ...
    def new_functionCFG(self, node, asynchr=False, enclosing_block_id=-1):
        ...
        # FIXME 将该CFG中的all_variable列表，即与“model_load”有关的变量放入到子函数的CFG中
        self.cfg.functioncfgs[(enclosing_block_id, node.name)].all_variable = self.cfg.all_variable
        if node.name in self.cfg.function_related_Parameters.keys():
            self.cfg.functioncfgs[(enclosing_block_id, node.name)].all_variable |= self.cfg.function_related_Parameters[node.name]
    ...
    def new_ClassCFG(self, node, asynchr=False):
        ...
        # FIXME 将该CFG中的all_variable列表，即与“model_load”有关的变量放入到子类的CFG中
        self.cfg.class_cfgs[node.name].all_variable = self.cfg.all_variable
    ...
    def visit_Try(self, node):
        ...
        #create a new block for else body of try
        else_block = self.new_block()
        # from # try to else 
        self.add_exit(try_block, else_block, ast.Constant(True))
