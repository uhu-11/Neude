# 报错信息路径：/media/lzq/D/lzq/pylot_test/pylot/error_infos/{i}.txt，i代表迭代的次数。文件中存储了报错信息。没有出现的i代表没有出现报错，不需要计算。
# 向量路径：/media/lzq/D/lzq/pylot_test/pylot/cov_vector/{i}_array_vector.npy，i代表迭代的次数。每个npy中包含若干数组，每个数组存储了对应名称的html文件的覆盖向量。

# 主要任务：计算"触发相同错误的用例"和"触发不同错误的用例"的覆盖向量之间的余弦相似度。
# 要求：1.计算覆盖率之前，先整合两个比较的npy文件中的数组，把两个npy文件中的数组整合为一个数组，注意数组名称顺序要一一对应，否则就计算不了正确的余弦相似度，如果某个npy文件缺少另一个npy中有的数组，则用0填充，并记录下缺失的数组名称。确保二者合并的数组名称、长度相同，防止在计算余弦相似度的过程中出现向量长度不一致的错误。
# 注意整合时排除数组名称包含pythonfuzz的数组。并且排除值为-1的位置（-1代表空行和注释，对覆盖向量没有意义）。
# 2.计算"触发相同错误的用例"和"触发不同错误的用例"的覆盖向量之间的余弦相似度。
# 3.打印两个npy文件整合后的数组的长度、和余弦相似度结果。
